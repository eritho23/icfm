...
class CmdCB : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *c) override {
        ...
        portENTER_CRITICAL(&g_mux);
        // Drop incoming command if previous one hasn't been consumed
        // yet. Main loop should be polling fast enough that this never
        // happens.
        if (!g_cmd_ready) {
...
void ble_init(void) {
    ...
    g_log_char = svc->createCharacteristic(BLE_LOG_CHAR_UUID,
