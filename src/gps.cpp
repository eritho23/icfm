...
void gps_setup() {
    // Assume that the U-Blox GNSS is running at 9600 baud (the default) or
    // at 38400 baud. Loop until we're in sync and then ensure it's at 38400
    // baud.
    do {
        ...
        if (gps.begin(Serial1) == true) {
            ...
            // gps.factoryReset();
            delay(2000); // Wait a bit before trying again to limit
                         // the Serial output
        }
    } while (1);
