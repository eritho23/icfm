...
static void _build_F(f32 dt, f32 wx, f32 wy, f32 wz) {
    ...
    set_F(p_x, p_x, 1); set_F(p_x, v_x, dt); set_F(p_x, a_x, d2);
    set_F(p_y, p_y, 1); set_F(p_y, v_y, dt); set_F(p_y, a_y, d2);
    set_F(p_z, p_z, 1); set_F(p_z, v_z, dt); set_F(p_z, a_z, d2);
    set_F(v_x, v_x, 1); set_F(v_x, a_x, dt);
    set_F(v_y, v_y, 1); set_F(v_y, a_y, dt);
    set_F(v_z, v_z, 1); set_F(v_z, a_z, dt);
    set_F(a_x, a_x, 1);
    set_F(a_y, a_y, 1);
    set_F(a_z, a_z, 1);
    set_F(d_theta, d_theta,  1);     set_F(d_theta, d_alpha,  wz*dt); set_F(d_theta, d_beta, -wy*dt);
    set_F(d_alpha, d_theta, -wz*dt); set_F(d_alpha, d_alpha,  1);     set_F(d_alpha, d_beta,  wx*dt);
    set_F(d_beta,  d_theta,  wy*dt); set_F(d_beta,  d_alpha, -wx*dt); set_F(d_beta,  d_beta,  1);
}
...
static void _build_I(void) {
    mat_clear(&kf.I);
    for (int i = 0; i < state_dim; i++) kf.I.data[i * state_dim + i] = 1.0f;
}
...
static void _compute_gain(void) {
    ...
    for (int i = 0; i < m_dim; i++) kf.S.data[i*m_dim + i] += 1e-6f;
...
b32 kalman_filter_init(const matrix *init_state, quat init_q, f32 init_variance) {
    ...
    // Initial covariance: large for translational states, tight for attitude error
    f32 var[state_dim];
    for (int i = 0; i < state_dim; i++) var[i] = init_variance;
    var[d_theta] = 1e-4f;
    var[d_alpha] = 1e-4f;
    var[d_beta]  = 1e-4f;
...
void kalman_filter_debug_print_csv_row(u32 t_ms, f32 wx, f32 wy, f32 wz, const matrix *state) {
    ...
    Serial.print(','); Serial.print(kf.q_ref.w, 6);
    Serial.print(','); Serial.print(kf.q_ref.x, 6);
    Serial.print(','); Serial.print(kf.q_ref.y, 6);
    Serial.print(','); Serial.print(kf.q_ref.z, 6);
    Serial.println();
}
