...
static void _build_H(b32 use_gps) {
    ...
    // NOTE: Do not use vertical acceleration since we cannot know due to
    // gravity and rocket thrust being in the same axis.
...
b32 kalman_filter_init(const matrix *init_state, quat init_q,
    ...
    // Initial covariance: large for translational states, tight for
    // attitude error
    f32 var[state_dim];
    for (int i = 0; i < state_dim; i++)
        var[i] = init_variance;
...
void kalman_filter_debug_print_csv_row(u32 t_ms, f32 wx, f32 wy, f32 wz,
    ...
    ble_sendf("%lu,%.6f,%.6f,%.6f,"
              "%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,"
              "%.6f,%.6f,%.6f,%.6f",
              (unsigned long)t_ms, wx, wy, wz, state->data[p_x],
              state->data[p_y], state->data[p_z], state->data[v_x],
              state->data[v_y], state->data[v_z], state->data[a_x],
              state->data[a_y], state->data[a_z], state->data[d_theta],
              state->data[d_alpha], state->data[d_beta], kf.q_ref.w, kf.q_ref.x,
              kf.q_ref.y, kf.q_ref.z);
}
